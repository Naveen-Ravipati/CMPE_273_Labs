...
export default class Assignment_details extends Component {
  ...
  my_Callback = (e) => {
    this.setState({
      submit_visibility: "hidden",
      alternate_submit: "visible",
      button_text: "Resubmit Assignment"
    })
  }
    ...
    if (this.state.assignment_detail) {
      var assign_det = this.state.assignment_detail.map(assignment => {
        return (
          ...
          <div>
            <h3>{assignment.name}
              <button onClick={this.pop_submit} class="btn btn-primary" style={{ float: "right", visibility: this.state.alternate_submit }} type="button">{this.state.button_text}</button>
            </h3>
    ...
    return (
      <div>
        <div class="container border" style={{ width: "80%" }}>
          {assign_det}
          <div class="border" style={{ visibility: this.state.submit_visibility, marginTop: "10%" }}>
            <Submitassignment callbackFromParent={this.my_Callback} />
          </div>
        </div>
