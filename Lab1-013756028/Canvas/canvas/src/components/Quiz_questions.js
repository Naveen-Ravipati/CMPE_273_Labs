...
class Quiz_questions extends Component {
    ...
    changehandler = async(e) =>{
        ...
        console.log('aaaaaaa'+JSON.stringify( this.state.quiz_responses))
    ...
    componentDidMount=async()=>{
       ...
       await axios.post('http://localhost:3001/quiz_questions',data)
                ...
                //update the state with the response data
                    ...
                    if(this.state.quiz_questions == ''){
                    await this.setState({
                        quiz_questions : this.state.quiz_questions.concat(response.data)
                    })}
              await console.log('here :'+this.state.quiz_questions[0])
            });
    }
